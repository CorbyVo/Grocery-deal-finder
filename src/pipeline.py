...
def run_pipeline(user_shopping_list, user_latitude, user_longitude, threshold=0.50):
    """
    Full pipeline:
    1. Match raw user input to normalized product names
    2. Keep unmatched user inputs separately
    3. Recommend best store
    """
    matcher = ProductMatcher()

    # version 1: remove duplicates but keep original order
    user_shopping_list = list(dict.fromkeys(user_shopping_list))
    ...
    for item in user_shopping_list:
        match_result = matcher.match_product_detailed(item, threshold=threshold)
    ...
    matched_details = [
        row for row in matching_details
        if row["matched_product"] is not None
    ]
    ...
    matched_offers, basket_summary, cheapest_store, nearest_store, message = recommend_store(
        matched_details,
